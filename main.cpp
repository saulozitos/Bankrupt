...
int main()
{
    ...
    for(unsigned short match = 1; match <= totalMatch; ++match)
    {
        ...
        if(isGameTimeout || game->isGameOver())
        {
            ...
            if(isGameTimeout)
            {
                std::cout<< "In simulation #" << match << " the game ended by time out - The winner is " << playerName << std::endl;
                countWins[playerName].first += 1;
            }
            else
            {
                std::cout<< "In simulation #" << match << " the game ended in round " << round << " - The winner is " << playerName << std::endl;
